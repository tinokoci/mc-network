...
public class SpeedTwoPotionsListener extends ConfigListener {
    ...
    public void onPotionSplashEvent(PotionSplashEvent event) {
        ...
        if (potion.getType() == PotionType.SPEED) {
            if (ToggleOption.SPEED_POTIONS_I.isEnabled() && potion.getLevel() > 1) {
    ...
    public void onPlayerItemConsume(PlayerItemConsumeEvent event) {
        ...
        if (potion.getType() == PotionType.SPEED && potion.getLevel() > 1) {
            ...
            player.sendMessage(CC.RED + "You cannot use " + CC.BOLD + "Speed II" + CC.RED + " potions because they are disabled.");
        }
    }
}

...
public class BorderData
{
    ...
    BorderData(double x, double z, int radiusX, int radiusZ, Boolean shapeRound, boolean wrap)
    {
    ...
    BorderData(double x, double z, int radiusX, int radiusZ, Boolean shapeRound)
    {
    ...
    private void setData(double x, double z, int radiusX, int radiusZ, Boolean shapeRound, boolean wrap)
    {
        this.x = x;
        this.z = z;
    ...
    private void setData(double x, double z, int radiusX, int radiusZ, Boolean shapeRound)
    {
        setData(x, z, radiusX, radiusZ, shapeRound, false);
    }
    private void setData(double x, double z, int radius, Boolean shapeRound)
    {
    ...
    /**
     * Replaced by {@link #getRadiusX()} and {@link #getRadiusZ()};
    ...
    public boolean insideBorder(double xLoc, double zLoc, boolean round)
    {
        ...
        else
        {
            ...
            else // After further calculation, inside
                // Apparently outside, then
                return X * X + Z * Z * radiusSquaredQuotient < radiusXSquared;
        }
    }
    ...
    private static final LinkedHashSet<Integer> safeOpenBlocks = new LinkedHashSet<>(List.of(
            0, 6, 8, 9, 27, 28, 30, 31, 32, 37, 38, 39, 40, 50, 55, 59, 63, 64, 65, 66, 68, 69, 70, 71, 72, 75, 76, 77, 78, 83, 90, 93, 94, 96, 104, 105, 106, 115, 131, 132, 141, 142, 149, 150, 157, 171));

    //these material IDs are ones we don't want to drop the player onto, like cactus or lava or fire or activated Ender portal
    private static final LinkedHashSet<Integer> painfulBlocks = new LinkedHashSet<>(List.of(
            10, 11, 51, 81, 119));
    ...
    {
        boolean safe = safeOpenBlocks.contains(world.getBlockTypeIdAt(X, Y, Z))		// target block open and safe
                && safeOpenBlocks.contains(world.getBlockTypeIdAt(X, Y + 1, Z));	// above target block open and safe
        if (!safe || flying)
            return safe;

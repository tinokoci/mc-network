package net.exemine.core.util.particle;
...
public enum ParticleEffect {
    ...
    /** Determine if the data helper for a particle effect is correct
     * @param effect
     *            Particle effect
     * @param data
     *            Particle data
     * @return Whether the data helper is correct or not */
    ...
    /** Determine if the color helper for a particle effect is correct
     * @param effect
     *            Particle effect
     * @param color
     *            Particle color
     * @return Whether the color helper is correct or not */
     ...
     *             If the particle effect isn't supported by the server version
    ...
    public void display(float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center, Player... players)
            throws ParticleVersionException, ParticleDataException, IllegalArgumentException {
        display(offsetX, offsetY, offsetZ, speed, amount, center, List.of(players));
    }
     ...
     *             If the particle effect requires additional data
     * @throws IllegalArgumentException
     *             If the particle effect isn't directional or if it requires water and none is at the center location
    ...
    public void display(Vector direction, float speed, Location center, Player... players) throws ParticleVersionException,
            ParticleDataException, IllegalArgumentException {
        display(direction, speed, center, List.of(players));
    }
     ...
     *             If the particle effect isn't colorable or the color helper is incorrect
    ...
    public void display(ParticleColor color, Location center, Player... players) throws ParticleVersionException, ParticleColorException {
        display(color, center, List.of(players));
    }
     ...
     *             If the particle effect isn't supported by the server version
     * @throws ParticleDataException
     *             If the particle effect does not require additional data or if the data helper is incorrect
     * @see ParticlePacket
     * @see ParticlePacket#sendTo(Location, double) */
    ...
    public void display(ParticleData data, float offsetX, float offsetY, float offsetZ, float speed, int amount, Location center,
        ...
        display(data, offsetX, offsetY, offsetZ, speed, amount, center, List.of(players));
    }
    ...
    public void display(ParticleData data, Vector direction, float speed, Location center, Player... players)
            throws ParticleVersionException, ParticleDataException {
        display(data, direction, speed, center, List.of(players));
    }
    ...
    /** Represents a runtime exception that is thrown either if the displayed particle effect isn't colorable or if the particle color helper
     * is incorrect
     * <p>
     * This class is part of the <b>ParticleEffect Library</b> and follows the same usage conditions
     * @author DarkBlade12
    ...
    public static final class ParticlePacket {
         ...
         *             if your bukkit version isn't supported by this library */
        public static void initialize() throws VersionIncompatibleException {
            ...
            try {
                ...
                Class<?> packetClass = ParticleReflection.PackageType.MINECRAFT_SERVER.getClass(version < 7 ? "Packet63WorldParticles"
                ...
                sendPacket = ParticleReflection.getMethod(playerConnection.getType(), "sendPacket",
                        ParticleReflection.PackageType.MINECRAFT_SERVER.getClass("Packet"));
        ...
        private void initializePacket(Location center) throws PacketInstantiationException {
            ...
            try {
                ...
                if(version < 8) {
                    ...
                    ParticleReflection.setValue(packet, true, "a", name);
                } else {
                    ParticleReflection.setValue(packet, true, "a", enumParticle.getEnumConstants()[effect.getId()]);
                    ParticleReflection.setValue(packet, true, "j", longDistance);
                    if(data != null) {
                        int[] packetData = data.getPacketData();
                        ParticleReflection.setValue(packet, true, "k", effect == ParticleEffect.ITEM_CRACK ? packetData
                                : new int[] { packetData[0] | (packetData[1] << 12) });
                    }
                }
                ParticleReflection.setValue(packet, true, "b", (float) center.getX());
                ParticleReflection.setValue(packet, true, "c", (float) center.getY());
                ParticleReflection.setValue(packet, true, "d", (float) center.getZ());
                ParticleReflection.setValue(packet, true, "e", offsetX);
                ParticleReflection.setValue(packet, true, "f", offsetY);
                ParticleReflection.setValue(packet, true, "g", offsetZ);
                ParticleReflection.setValue(packet, true, "h", speed);
                ParticleReflection.setValue(packet, true, "i", amount);
            } catch (Exception exception) {
        ...
        public void sendTo(Location center, double range) throws IllegalArgumentException {
            ...
            for (Player player : Bukkit.getOnlinePlayers()) {
                if(!player.getWorld().getName().equals(worldName) || player.getLocation().distanceSquared(center) > squared || !Core.get().getUserService().get(player).getData().isParticles()) {
                    continue;
                }
        ...
        /** Represents a runtime exception that is thrown if a bukkit version isn't compatible with this library
         * <p>
         * This class is part of the <b>ParticleEffect Library</b> and follows the same usage conditions
         * @author DarkBlade12
         * @since 1.5 */

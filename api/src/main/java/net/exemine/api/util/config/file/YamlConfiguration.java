...
package net.exemine.api.util.config.file;

import net.exemine.api.util.config.configuration.Configuration;
import net.exemine.api.util.config.configuration.ConfigurationSection;
import net.exemine.api.util.config.configuration.InvalidConfigurationException;
import net.exemine.api.util.config.serialization.ConfigurationSerializable;
import net.exemine.api.util.config.serialization.ConfigurationSerialization;
...
public class YamlConfiguration extends FileConfiguration {
    ...
    public void saveResource(ClassLoader classLoader, File dataFolder, String resourcePath, boolean replace) {
        ...
        if (in == null) {
            throw new IllegalArgumentException(
                    "The embedded resource '" + resourcePath + "' cannot be found in the parent directory");
        }
        ...
        try {
            if (!outFile.exists() || replace) {
                ...
                Logger.getLogger(YamlConfiguration.class.getName()).log(Level.WARNING, "Could not save " + outFile.getName() + " to " + outFile + " because "
                        + outFile.getName() + " already exists.");
            }
        } catch (IOException ex) {
            Logger.getLogger(YamlConfiguration.class.getName()).log(Level.SEVERE, "Could not save " + outFile.getName() + " to " + outFile, ex);
        }
    }

    public InputStream getResource(ClassLoader classLoader, String filename) {
        if (filename == null) {
